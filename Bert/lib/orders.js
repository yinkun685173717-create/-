...
async function supabaseRequest(path, options = {}) {
  ...
  if (!config) {
    throw new Error("Supabase is not configured. 请检查 Vercel 环境变量 SUPABASE_URL 和 SUPABASE_SERVICE_ROLE_KEY。");
  }
...
async function getOrders({ localStore } = {}) {
  ...
  if (!localStore) {
    throw new Error("Supabase is not configured. 请检查 Vercel 环境变量。");
  }
